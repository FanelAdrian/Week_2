...
//function zodiac(day:number, month:string){
    ...
    } else console.log("te has inventado esa fecha.")
}

let europa: string[] = ["Rumania", "España", "Portugal", "Malta", "Irlanda"];
let asia: string[] = ["Irak", "Rusia", "Taiwan", "China", "Corea del Norte"];
let america: string[] = ["Peru", "Canada", "Groenlandia", "Puerto Rico", "Barbados"];
let oceania: string[] = ["Australia", "Papua Nueva Guinea", "Nueva Zelanda", "Samoa", "Tuvalu"];
let africa: string[] = ["Guinea Ecuatorial", "Sudafrica","Egipto","Madagascar", "Niger"];


const continent = (country:string) => {
    // en este caso estamo creando una funcion que puede tener dos valosres y lo indicamos con |.
    let inEurope: string | undefined = europa.find((europeCountry: string) => country === europeCountry);
    if (inEurope !== undefined){
        console.log("Esta en Europa");
    }
    else if (america.find((americanCountry: string) => country === americanCountry)) {
        console.log("Esta en America");
    }
    else if (asia.find((asianCountry: string) => country === asianCountry)) {
        console.log("Esta en Asia");
    }
    else if (oceania.find((oceanianCountry: string) => country === oceanianCountry)) {
        console.log("Esta en Oceanía");
    }
    else if (africa.find((africanCountry: string) => country === africanCountry)) {
        console.log("Esta en Africa");
    }else console.log("No se que país es ese.")
}
continent("Taiwan");
continent("Africa");
